package ru.explosive.caremom.voice
...
class VoiceToTextParser(private val app: Application) : RecognitionListener {
    ...
    fun startListening(languageCode: String = "ru-RU") {
        ...
        if (!SpeechRecognizer.isRecognitionAvailable(app)) {
            _state.update { it.copy(error = "Распознавание речи недоступно", isSpeaking = false) }
            return
        }
    ...
    override fun onError(error: Int) {
        ...
        _state.update { it.copy(error = "Ошибка: $error", isSpeaking = false) }
    }
